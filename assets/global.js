...
class DeferredMedia extends HTMLElement {
  constructor() {
    ...
    if(autoplay) {
      this.loadContent(true, autoplay);
    }
  }

  loadContent(focus = true, autoplay) {
    // window.pauseAllMedia();
    if (autoplay) {
...
class VariantSelects extends HTMLElement {
  ...
  onVariantChange() {
    ...
    if (!this.currentVariant) {
      ...
      this.updateShareUrl();
      this.updatePaymentInstalmentMsg();
    }
  }
  ...
  updatePaymentInstalmentMsg() {
    const ELM_payment_instalment_msg_val = document.getElementById('payment-instalment-msg-val');
    if (this.currentVariant) {
      const priceInstalment = (this.currentVariant.price / 100 / 4).toFixed(2);
      if (ELM_payment_instalment_msg_val) {
        ELM_payment_instalment_msg_val.innerHTML = priceInstalment;
      }
    }
  }
  ...
  updateMasterId() {
    this.currentVariant = this.getVariantData().find((variant) => {
      const results = variant.options.map(option => {
  ...
  updateURL() {
    ...
    // variantsArr from snippets/product-info.liquid
    let isReload = false;
    for (let i in variantsArr) {
      const variant = variantsArr[i];
      if (variant.id === this.currentVariant.id && variant.hidden) {
        isReload = true;
        break;
      }
    }
  ...
  toggleAddButton(disable = false, text, modifyClass = true) {
    ...
    const productFormBtns = document.getElementById('product-form-buttons');
    ...
    if (disable) {
      ...
      productFormBtns.classList.remove('hide-back-in-stock-button');
    } else {
      addButton.removeAttribute('disabled');
      addButtonText.textContent = addButton.getAttribute('data-text');
      productFormBtns.classList.add('hide-back-in-stock-button');
    }
...
class VariantRadios extends VariantSelects {
  ...
  updateOptions() {
    ...
    this.options = fieldsets.map((fieldset) => {
      const selectedVal = Array.from(fieldset.querySelectorAll('input')).find((radio) => radio.checked).value;
      const selectedSize = fieldset.querySelector('[data-select-size]');
      if (selectedSize) {
        selectedSize.innerHTML = selectedVal;
      }
      return selectedVal;
    });
  }
